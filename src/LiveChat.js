...
function Livechat() {
  ...
  const handleSend = (e) => {
    ...
    const msgData = {
      userId: "userId123",  // Gantilah dengan ID pengguna yang sesungguhnya jika ada
    ...
    if (isInFAQ) {
      ...
      if (message === "1") {
        ...
        // If "menu" is typed, return to the main menu from FAQ
        setIsInFAQ(false);
        setChat((prevChat) => [
          ...prevChat,
          {
  ...
  const handleEndChat = () => {
    ...
    setIsUserInfoSet(false); // Reset user info state
    setChatStarted(false); // Reset chat started status
    setIsInFAQ(false); // Reset FAQ mode
